<template>
  ...
  <div class="flex flex-col items-center justify-center h-64 bg-brand-gray">
    ...
    <p class="mt-1 text-lg text-center text-gray-800 font-regular">
    ...
    <div class="w-4/5 max-w-6xl py-10 grid grid-cols-4 gap-2">
      <div>
        ...
        <suspense>
          <template #default>
            <filters
              @select="handleChangeFeedbacksType"
...
interface IState {
  ...
  isLoadingMoreFeedbacks: boolean
  feedbacks: Feedback[]
...
const { handleFeedbacks } = useFeedbacks()

onErrorCaptured(handleErrors)

onMounted(() => {
  handleFeedbacksAdapter()
...
async function handleFeedbacksAdapter() {
  try {
    state.isLoading = true

    const data = await handleFeedbacks({
      ...state.pagination,
...
async function handleChangeFeedbacksType(type: FeedbackType) {
  try {
